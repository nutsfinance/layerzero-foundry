...
import "lib/@layerzero/contracts/lzApp/libs/LzLib.sol";
import "./interfaces/ILayerZeroReceiver.sol";
import "./interfaces/ILayerZeroEndpoint.sol";
import {console} from "lib/forge-std/src/console.sol";

contract LZEndpointMock is PRBTest, ILayerZeroEndpoint {
    ...
    mapping(uint16 => mapping(uint64 => DataTypes.sendData)) public messages;
    ...
    modifier receiveNonReentrant() {
        require(
            _receive_entered_state == _NOT_ENTERED, "LayerZeroMock: no receive reentrancy"
        );
    ...
    event PayloadCleared(
        uint16 srcChainId, bytes srcAddress, uint64 nonce, address dstAddress
    );
    event PayloadStored(
        uint16 srcChainId,
        bytes srcAddress,
        ...
        sendNonReentrant
    {
...

        {
            ...
            (, uint256 extra, uint256 dstNativeAmt, address payable dstNativeAddr) =
        ...
        messages[_chainId][nonce] = DataTypes.sendData({
            targetChainId: _chainId,
            sourceAddress: srcUaAddress,
            targetAddress: dstAddr,
            nonce: nonce,
            extraGas: extraGas,
            payload: _payload
        });
    ...
    function getSendData(
        ...
        public
        view
        returns (DataTypes.sendData memory data)
    {
        data = messages[targetChainId][nonce];
    }
        ...
        receiveNonReentrant
    {
        ...
        require(
            _nonce == ++inboundNonce[_srcChainId][_path], "LayerZeroMock: wrong nonce"
        );
        ...
        returns (uint256 nativeFee, uint256 zroFee)
    {
        ...
        uint256 relayerFee = _getRelayerFee(
            _dstChainId, 1, _userApplication, _payload.length, adapterParams
        );
    ...
    function _clearMsgQue(uint16 _srcChainId, bytes calldata _path) internal {
        ...
        while (msgs.length > 0) {
            ...
            ILayerZeroReceiver(payload.dstAddress).lzReceive(
                _srcChainId, _path, payload.nonce, payload.payload
            );
            msgs.pop();
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 pricePerByte = (
            relayerFeeConfig.dstGasPriceInWei * relayerFeeConfig.gasPerByte
